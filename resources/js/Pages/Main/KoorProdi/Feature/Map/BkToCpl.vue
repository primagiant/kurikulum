<template>
    ...
    <div>
        ...
        <div class="p-6">
            <h1 class="mb-5">Bahan Kajian - Capaian Pembelajaran Lulusan</h1>
            <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400 mb-4">
                        <thead class="text-xs text-gray-700 bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                            <tr>
                                <th scope="col" rowspan="2" class="px-4 py-3 text-center">Kode BK</th>
                                <th scope="col" colspan="100%" class="px-4 py-3 text-center">CPL (Capaian Profil Lulusan)
                                </th>
                            </tr>
                            <tr>
                                <th v-for="cpl_item in cpl" :data-popover-target="`popover-cpl-${cpl_item.id_cpl}`"
                                    scope="col" class="px-4 py-3 text-center cursor-help">
                                    {{ cpl_item.kode_cpl }}
                        ...
                        <tbody>
                            <tr v-for="bk_item, idy in bk" class="border-b dark:border-gray-700">
                                <td class="px-4 py-3 text-center cursor-help"
                                    :data-popover-target="`popover-bk-${bk_item.id_bk}`" data-popover-placement="right">
                                    {{ bk_item.kode_bk }}
                                    <div data-popover :id="`popover-bk-${bk_item.id_bk}`" role="tooltip"
                                        class="absolute z-10 font-light invisible inline-block w-64 text-sm text-gray-500 transition-opacity duration-300 bg-white border border-gray-200 rounded-lg shadow-sm opacity-0 dark:text-gray-400 dark:border-gray-600 dark:bg-gray-800">
                                        <div
