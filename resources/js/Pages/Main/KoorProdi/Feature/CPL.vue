<template>
    ...
    <layout>
        ...
        <div class="p-6">
            <h1 class="mb-5">Capaian Profil Lulusan List</h1>
                    ...
                    <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
                        ...
                        <div class="overflow-x-auto">
                            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                                ...
                                <tbody>
                                    <tr v-for="capaian in cpl.data" :key="cpl.id_cpl" class="border-b dark:border-gray-700">
                                        <th scope="row"
                                            class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                            {{ capaian.kode_cpl }}
                                        </th>
                                        <td class="px-4 py-3">{{ capaian.unsur }}</td>
                                        <td class="px-4 py-3">{{ capaian.deskripsi_cpl }}</td>
                                        <td class="px-4 py-3">{{ capaian.referensi }}</td>
                                        <td class="px-4 py-3 flex items-center justify-end gap-2">
                                            <button type="button"
                                                ...
                                                @click="activateUpdateMode(capaian)">
                                            ...
                                            <button type="button"
                                                ...
                                                @click="deleteCPL(capaian.id_cpl)">
                                                <svg width="16" fill="none" stroke="currentColor" stroke-width="1.5"
                                                    viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"
                                                    aria-hidden="true">
                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                    ...
                                    <tr v-if="cpl.data.length == 0">
                                        <td colspan="4" class="px-4 py-3 font-medium text-gray-900 text-center">No Data</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
                        <Pagination v-if="cpl.data.length != 0" :links="cpl.links" :from="cpl.from" :to="cpl.to"
                            :total="cpl.total" />
                    </div>
                </div>
                    ...
                    <form @submit.prevent="submit" autocomplete="off"
                        ...
                        <div class="mb-6 w-full">
                            ...
                            <textarea id="default-deskripsi" rows="3" v-model="form.deskripsi_cpl" required
                                ...
                                placeholder="Tulis Deskripsi Capaian Profil Lulusan.."></textarea>
...
const updatedCPL = ref(null)
const activateUpdateMode = (cpl) => {
    updateMode.value = true
    updatedCPL.value = cpl.id_cpl
    form.deskripsi_cpl = cpl.deskripsi_cpl
    form.unsur = cpl.unsur
    form.referensi = cpl.referensi
}
...
const submit = () => {
    if (updateMode.value) {
        form.post(route('cpl.update', { id: updatedCPL.value }), {
            preserveScroll: true,
            onSuccess: () => {
                updateMode.value = false
                updatedCPL.value = null
                form.reset()
            },
        })
    } else {
...
const deleteCPL = (id) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            router.delete(route('cpl.destroy', { id: id }), {
                preserveScroll: true,
                preserveState: true,
            })
        }
